...
import { collection, query, where, onSnapshot, doc, updateDoc, getDocs, setDoc, serverTimestamp } from "firebase/firestore";
...
export default function PayrollPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setEmployees(snapshot.docs.map(d => ({ id: d.id, ...d.data() } as EmployeePayroll)));
    });
    return () => unsubscribe();
  }, [isAdmin]);
  ...
  const handleGeneratePayroll = async () => {
    ...
    try {
      ...
      toast.error("Failed to generate payroll calculations.");
    } finally {
      setIsGenerating(false);
    }
  };

  // 🔥 NEW: Deterministic Anti-Duplication Payslip Distribution Function
  const handleDistributePayslip = async (emp: EmployeePayroll, result: PayrollResult) => {
    ...
    try {
      await setDoc(doc(db, "payslips", deterministicId), {
        userId: emp.id,
        ...
        authorizedBy,
        metrics: {
          semiMonthlyBase: result.semiMonthlyBase,
          hourlyRate: result.hourlyRate,
          daysPresent: result.daysPresent,
          paidLeaveDays: result.paidLeaveDays,
          unpaidAbsences: result.unpaidAbsences,
          absenceDeductionPeso: result.absenceDeductionPeso,
          totalLateMins: result.totalLateMins,
          totalUndertimeMins: result.totalUndertimeMins,
          timePenaltiesPeso: result.timePenaltiesPeso,
          totalOtMinutes: result.totalOtMinutes,
          otPayPeso: result.otPayPeso,
          sss: result.sss,
      ...
      toast.success(`Payslip securely pushed to ${emp.fullName}'s 201 File!`);
    } catch (error) {
      console.error("Distribution Error:", error);
      toast.error("Failed to transmit statement record.");
    } finally {
      setIsDistributing(prev => ({ ...prev, [emp.id]: false }));
    }
  };
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-12 print:hidden">
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
              <Banknote className="w-10 h-10 text-emerald-500" /> Payroll Management
            </h1>
            <p className="text-gray-500 dark:text-gray-400 mt-2">Automated semi-monthly salary generation, OT calculations, and deductions.</p>
          </div>
            ...
            <div className="flex flex-wrap items-center gap-4 w-full xl:w-auto">
              <div className="flex flex-col gap-1.5 flex-1 md:flex-none">
                ...
                <div className="relative">
                  ...
                  <input type="month" value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="bg-gray-50 dark:bg-black/40 border border-gray-200 dark:border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500 w-full" />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5 w-full md:w-auto pt-1 md:pt-0">
                ...
                <label className="flex items-center gap-3 bg-gray-50 dark:bg-black/40 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 cursor-pointer hover:border-emerald-500/50 transition-colors h-[42px]">
                  <input type="checkbox" checked={applyDeductions} onChange={(e) => setApplyDeductions(e.target.checked)} className="w-4 h-4 text-emerald-600 rounded focus:ring-emerald-500" />
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 select-none">Apply Gov. Deductions</span>
                </label>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-3xl p-6 shadow-xl overflow-x-auto">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Search employee..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-2 bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl text-sm outline-none text-gray-900 dark:text-white focus:ring-2 focus:ring-emerald-500 transition-all" />
              </div>
            </div>
            ...
            <table className="w-full text-left min-w-[1350px]">
              <thead>
                <tr className="text-[10px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-200 dark:border-white/5">
                  ...
                  <th className="pb-4 text-right pl-4">Earnings (Base + OT)</th>
                  ...
                  <th className="pb-4 text-center w-48">Actions</th>
                </tr>
              </thead>
                      ...
                      <td className="py-4 text-right font-medium text-gray-700 dark:text-gray-300">
                        {editingId === emp.id ? (
                          <div className="flex items-center justify-end gap-2">
                            <input type="number" value={editSalary} onChange={(e) => setEditSalary(Number(e.target.value))} className="w-24 px-2 py-1 bg-white dark:bg-black/40 border rounded text-right text-gray-900 dark:text-white" />
                            <select value={editType} onChange={(e) => setEditType(e.target.value as "monthly" | "hourly")} className="px-2 py-1.5 bg-white dark:bg-black/40 border rounded text-xs text-gray-900 dark:text-white">
                              <option value="monthly">/mo</option>
                              <option value="hourly">/hr</option>
                            </select>
                            <button onClick={() => handleSaveSalary(emp.id)} disabled={isSaving} className="text-emerald-500"><Save className="w-4 h-4" /></button>
                            <button onClick={() => setEditingId(null)} className="text-gray-400"><XCircle className="w-4 h-4" /></button>
                          </div>
                        ) : (
                          <div className="flex items-center justify-end gap-3">
                            ...
                            <button onClick={() => handleEditClick(emp)} className="text-gray-400 hover:text-emerald-500 opacity-0 group-hover:opacity-100"><Edit2 className="w-3.5 h-3.5" /></button>
                          </div>
                        )}
                      </td>
                      ...
                      {result ? (
                        <>
                          <td className="py-4 text-right font-semibold text-gray-900 dark:text-white">
                            ...
                            {result.otPayPeso > 0 ? <div className="text-[10px] text-teal-600 font-bold mt-0.5">+ {formatPeso(result.otPayPeso)} OT ({result.totalOtMinutes}m)</div> : <div className="text-[10px] text-gray-400 font-normal mt-0.5">No OT Logged</div>}
                          </td>
                          <td className="py-4 text-right">
                            {(result.absenceDeductionPeso > 0 || result.timePenaltiesPeso > 0) ? (
                              <>
                                <span className="text-rose-600 dark:text-rose-400 font-bold">- {formatPeso(result.absenceDeductionPeso + result.timePenaltiesPeso)}</span>
                                <div className="text-[10px] text-gray-400 mt-0.5">{result.unpaidAbsences > 0 ? `${result.unpaidAbsences}d absent ` : ''}{result.timePenaltiesPeso > 0 ? `${result.totalLateMins + result.totalUndertimeMins}m lost` : ''}</div>
                              </>
                          ...
                          <td className="py-4 text-right">
                            ...
                            <div className="text-[10px] text-gray-400 mt-0.5 tracking-tighter">{!applyDeductions ? "Waived" : cutoffPeriod === 1 ? `SSS: ${formatPeso(result.sss)} | Pag-IBIG: ${formatPeso(result.pagibig)}` : `PH: ${formatPeso(result.philhealth)}`}</div>
                          </td>
                          <td className="py-4 text-right text-lg font-black text-emerald-600">{formatPeso(result.netPay)}</td>
                          <td className="py-4 text-center">
                            <div className="flex items-center justify-center gap-2">
                              <button onClick={() => setSelectedPayslip({ emp, result })} className="text-[10px] font-bold bg-slate-100 dark:bg-white/10 hover:bg-emerald-50 text-gray-600 dark:text-gray-300 hover:text-emerald-600 px-2.5 py-1.5 rounded-lg flex items-center gap-1"><FileText className="w-3 h-3" /> Preview</button>
                              {/* Integrated Distribution Action Control */}
                              <button onClick={() => handleDistributePayslip(emp, result)} disabled={isDistributing[emp.id]} className="text-[10px] font-bold bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-300 text-white px-2.5 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors shadow-sm">{isDistributing[emp.id] ? <span className="w-3 h-3 border border-white/20 border-t-white rounded-full animate-spin" /> : <Send className="w-3 h-3" />} Distribute</button>
                            </div>
                          </td>
                        </>
                      ) : <td colSpan={5} className="py-4 text-center border-l border-gray-100 dark:border-white/5"><span className="text-xs text-gray-400 italic flex items-center justify-center gap-2"><CheckCircle2 className="w-4 h-4 text-gray-300" /> Ready to compute</span></td>}
                    </tr>
                  );
                })}
              </tbody>
              ...
              <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full text-gray-900 mt-4 sm:mt-10 mb-4 sm:mb-10 print:m-0 print:shadow-none print:w-full print:max-w-none">
                <div className="flex flex-wrap sm:flex-nowrap justify-between items-center p-4 sm:p-5 border-b border-gray-200 print:hidden bg-slate-50 rounded-t-xl gap-3">
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <button onClick={() => setSelectedPayslip(null)} className="flex items-center gap-1.5 text-gray-600 hover:text-gray-900 bg-gray-200/50 px-3 py-2 rounded-lg transition-colors font-bold text-xs"><ArrowLeft className="w-4 h-4" /> Back</button>
                    <h3 className="font-bold text-gray-700 text-sm hidden sm:block">Payslip Preview</h3>
                  </div>
                  <button onClick={() => window.print()} className="flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg text-xs font-bold w-full sm:w-auto"><Printer className="w-4 h-4" /> Save as PDF</button>
                </div>
                ...
                <div className="p-5 sm:p-8 md:p-12 print:p-8 bg-white">
                  <div className="text-center mb-6 border-b-2 border-gray-900 pb-4">
                    ...
                    <p className="text-xs text-gray-600 mt-1">Subic City, Zambales</p>
                    <h2 className="text-base sm:text-lg font-bold mt-4 uppercase text-gray-800">Payslip</h2>
                    <p className="text-xs text-gray-600 font-semibold">Pay Period: {getCutoffLabel()}</p>
                  </div>

                  <div className="grid grid-cols-2 gap-4 mb-8 text-xs sm:text-sm">
                    <div><span className="font-bold text-gray-400 uppercase text-[10px] block mb-0.5">Employee Name</span><span className="font-semibold">{selectedPayslip.emp.fullName}</span></div>
                    <div><span className="font-bold text-gray-400 uppercase text-[10px] block mb-0.5">Department</span><span className="font-semibold">{selectedPayslip.emp.department || "N/A"}</span></div>
                    <div><span className="font-bold text-gray-400 uppercase text-[10px] block mb-0.5">Salary Profile</span><span className="font-semibold">{formatPeso(selectedPayslip.emp.baseSalary || 0)} / {selectedPayslip.emp.salaryType === 'hourly' ? 'Hr' : 'Mo'}</span></div>
                    <div><span className="font-bold text-gray-400 uppercase text-[10px] block mb-0.5">Hourly Rate</span><span className="font-semibold">{formatPeso(selectedPayslip.result.hourlyRate)}</span></div>
                  </div>
                  ...
                  <div className="w-full border border-gray-300 mb-8 rounded-lg overflow-hidden flex flex-col md:flex-row">
                    <div className="w-full md:w-1/2 flex flex-col border-b md:border-b-0 md:border-r border-gray-300">
                      <div className="bg-gray-100 p-3 font-bold uppercase text-[10px] border-b border-gray-300 text-gray-700">Earnings</div>
                      <div className="p-4 flex flex-col gap-3 min-h-[140px] text-xs">
                        <div className="flex justify-between"><span>Basic Pay (Gross)</span><span className="font-semibold">{formatPeso(selectedPayslip.result.semiMonthlyBase)}</span></div>
                        {selectedPayslip.result.otPayPeso > 0 && <div className="flex justify-between text-teal-700 font-medium"><span>Overtime Pay ({selectedPayslip.result.totalOtMinutes}m)</span><span className="font-semibold">+ {formatPeso(selectedPayslip.result.otPayPeso)}</span></div>}
                        <div className="flex justify-between text-gray-500 text-[10px] mt-auto pt-4 border-t"><span>Days Present: {selectedPayslip.result.daysPresent}</span>{selectedPayslip.result.paidLeaveDays > 0 && <span>Paid Leaves: {selectedPayslip.result.paidLeaveDays}</span>}</div>
                      </div>
                    ...
                    <div className="w-full md:w-1/2 flex flex-col">
                      ...
                      <div className="p-4 flex flex-col gap-3 min-h-[140px] text-xs">
                        {selectedPayslip.result.absenceDeductionPeso > 0 && <div className="flex justify-between text-rose-600"><span>Unpaid Absences ({selectedPayslip.result.unpaidAbsences}d)</span><span className="font-semibold">{formatPeso(selectedPayslip.result.absenceDeductionPeso)}</span></div>}
                        {selectedPayslip.result.timePenaltiesPeso > 0 && <div className="flex justify-between text-rose-600"><span>Lates/UT ({selectedPayslip.result.totalLateMins + selectedPayslip.result.totalUndertimeMins}m)</span><span className="font-semibold">{formatPeso(selectedPayslip.result.timePenaltiesPeso)}</span></div>}
                        {selectedPayslip.result.sss > 0 && <div className="flex justify-between"><span>SSS Contrib.</span><span className="font-semibold">{formatPeso(selectedPayslip.result.sss)}</span></div>}
                        {selectedPayslip.result.philhealth > 0 && <div className="flex justify-between"><span>PhilHealth Contrib.</span><span className="font-semibold">{formatPeso(selectedPayslip.result.philhealth)}</span></div>}
                        {selectedPayslip.result.pagibig > 0 && <div className="flex justify-between"><span>Pag-IBIG Contrib.</span><span className="font-semibold">{formatPeso(selectedPayslip.result.pagibig)}</span></div>}
                        {selectedPayslip.result.timePenaltiesPeso === 0 && selectedPayslip.result.absenceDeductionPeso === 0 && selectedPayslip.result.totalMandatory === 0 && <div className="text-gray-400 italic text-center my-auto">No deductions.</div>}
                      </div>
                      <div className="p-4 mt-auto border-t border-gray-300 bg-gray-50 flex justify-between font-bold text-rose-700 text-sm"><span>Total Deductions</span><span>{formatPeso(selectedPayslip.result.absenceDeductionPeso + selectedPayslip.result.timePenaltiesPeso + selectedPayslip.result.totalMandatory)}</span></div>
                    </div>
                  </div>

                  <div className="flex justify-between items-center bg-emerald-50 print:border p-4 rounded-lg mb-8">
                    <span className="text-sm font-black uppercase text-emerald-900 tracking-widest">Net Pay</span>
                    <span className="text-2xl font-black text-emerald-700 border-b border-gray-900">{formatPeso(selectedPayslip.result.netPay)}</span>
                  </div>

                  <div className="flex justify-between items-end gap-8 pt-8 px-4">
                    <div className="text-center w-48 border-t border-gray-800 pt-1 font-semibold text-xs">{selectedPayslip.emp.fullName}<div className="text-[9px] text-gray-400 uppercase font-bold tracking-wider mt-0.5">Employee Signature</div></div>
                    <div className="text-center w-48 border-t border-gray-800 pt-1 font-semibold text-xs group relative">{isEditingAuth ? <input type="text" value={authorizedBy} onChange={(e) => setAuthorizedBy(e.target.value)} onBlur={() => setIsEditingAuth(false)} onKeyDown={(e) => e.key === 'Enter' && setIsEditingAuth(false)} className="w-full text-center outline-none bg-emerald-50 text-xs" autoFocus /> : <div className="flex justify-center items-center gap-1"><span>{authorizedBy}</span><button onClick={() => setIsEditingAuth(true)} className="opacity-0 group-hover:opacity-100 text-gray-400"><Edit2 className="w-2.5 h-2.5" /></button></div>}<div className="text-[9px] text-gray-400 uppercase font-bold tracking-wider mt-0.5">Authorized Signatory</div></div>
                  </div>
                </div>
              </div>
            </div>
