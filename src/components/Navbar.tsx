...
export default function Navbar() {
  ...
  const getLinkClass = (path: string, isMobile = false) => { 
    ...
    const baseClass = isMobile 
      ? "w-full p-3 rounded-xl flex items-center gap-3 transition-all font-medium cursor-pointer" 
      : "p-2.5 rounded-xl flex items-center justify-center group transition-all active:scale-95 cursor-pointer";
      
    return `${baseClass} ${
      isActive 
        ? "text-teal-400 bg-teal-500/20 shadow-inner" 
        : "text-gray-300 hover:text-teal-400 hover:bg-white/10 active:bg-white/20"
    }`;
  };
  ...
  return (
    <nav className="fixed top-0 left-0 z-50 w-full border-b border-teal-500/10 bg-[#0b0f19]/90 backdrop-blur-md transition-colors duration-500">
      <div className="px-6 py-4 flex justify-between items-center max-w-[1600px] mx-auto">
        <div className="flex items-center gap-3">
          {/* Small Logo */}
          <Link href="/dashboard" className="relative w-10 h-10 flex items-center justify-center cursor-pointer">
            <Image 
          ...
          <Link href="/dashboard" className="cursor-pointer">
            <h1 className="text-xl font-light tracking-wider hidden sm:block text-gray-100 transition-colors">
              Simpli<span className="font-semibold text-teal-400">Sync</span>
            </h1>
          </Link>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-2">
          <div className="flex flex-col items-end mr-4 pr-4 border-r border-white/10">
            <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold transition-colors">Logged in as</span>
            <span className="text-xs text-gray-200 font-medium transition-colors">{user?.email}</span>
          </div>
          ...
          {isAdmin && (
            <div className="flex items-center gap-2 ml-2 pl-2 border-l border-white/10">
              <Link href="/admin/dashboard" className={getLinkClass("/admin/dashboard")} aria-label="Command Center" title="Command Center">
                <Activity className="w-5 h-5 text-emerald-500 transition-transform duration-300 group-hover:scale-110" />
              </Link>
              <Link href="/admin/memo-generator" className={getLinkClass("/admin/memo-generator")} aria-label="AI Policy Lab" title="Policy and Memo Lab">
                <Sparkles className="w-5 h-5 text-emerald-400 transition-transform duration-300 group-hover:scale-110" />
              </Link>
              ...
              <Link href="/admin/benefits" className={getLinkClass("/admin/benefits")} aria-label="Benefits" title="Benefits Administration">
                <Gift className="w-5 h-5 text-emerald-500 transition-transform duration-300 group-hover:scale-110" />
              </Link>
              <Link href="/admin/disciplinary" className={getLinkClass("/admin/disciplinary")} aria-label="Disciplinary Advisor" title="Disciplinary Advisor">
                <Gavel className="w-5 h-5 text-emerald-500 transition-transform duration-300 group-hover:scale-110" />
              </Link>
              <Link href="/admin/performance" className={getLinkClass("/admin/performance")} aria-label="Performance" title="Performance Management">
                <TrendingUp className="w-5 h-5 text-emerald-500 transition-transform duration-300 group-hover:scale-110" />
              </Link>
            </div>
          )}
        ...
        <div className="lg:hidden flex items-center gap-3">
          <button 
            ...
            className="p-2 text-gray-300 bg-white/5 rounded-xl hover:bg-white/10 transition-colors cursor-pointer"
          >
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden absolute top-[73px] left-0 w-full max-h-[80vh] overflow-y-auto bg-[#0b0f19] border-b border-teal-500/10 shadow-2xl px-6 py-4 flex flex-col gap-2">
          ...
          {isAdmin && (
            <>
              ...
              <Link href="/admin/dashboard" onClick={() => setIsMobileMenuOpen(false)} className={getLinkClass("/admin/dashboard", true)}>
                <Activity className="w-5 h-5 text-emerald-500" /> Command Center
              </Link>
              ...
              <Link href="/admin/benefits" onClick={() => setIsMobileMenuOpen(false)} className={getLinkClass("/admin/benefits", true)}>
                <Gift className="w-5 h-5 text-emerald-500" /> Benefits Administration
              </Link>
              <Link href="/admin/disciplinary" onClick={() => setIsMobileMenuOpen(false)} className={getLinkClass("/admin/disciplinary", true)}>
                <Gavel className="w-5 h-5 text-emerald-500" /> Disciplinary Advisor
              </Link>
              ...
              <Link href="/admin/memo-generator" onClick={() => setIsMobileMenuOpen(false)} className={getLinkClass("/admin/memo-generator", true)}>
                <Sparkles className="w-5 h-5 text-emerald-400" /> AI Policy Lab
              </Link>
            </>
          )}
