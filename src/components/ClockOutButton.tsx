...
export default function ClockOutButton() {
  ...
  const handleClockOut = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      // Need that precise 50m radius!
    );
    console.log("Requested geolocation with high accuracy for clock out.");
  };
  ...
  return (
    <div className="flex flex-col items-center gap-4 mt-4">
      <button
        ...
        disabled={isLoading}
        className={`px-8 py-3 rounded-xl text-white font-bold tracking-wide transition-all shadow-md active:scale-95 flex items-center gap-2
          ${isLoading 
            ? "bg-gray-400 cursor-not-allowed" 
            : "bg-gradient-to-r from-rose-500 to-red-600 hover:from-rose-600 hover:to-red-700 shadow-rose-500/30 hover:shadow-rose-500/50"
          }`}
      >
        {isLoading ? (
          <>
